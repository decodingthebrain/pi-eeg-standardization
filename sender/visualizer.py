...
def the_input_is_valid(input_list):
  #  print('oks')
    global last_valid_value, counter
    msb = input_list[24]
    middle = input_list[25]
    lsb = input_list[26]

    current_value = _to_signed_24bit(msb, middle, lsb)
    
    if last_valid_value is None:
        _last_valid_value = current_value
        #print('here')
        return False

    difference = abs(current_value - last_valid_value)
    if difference > 5000:
        print(f'Corrupted data detected _counter: {counter}')
        counter += 1
        return False
...
while 1:
        ...
        if the_input_is_valid(output_2):
                #print('test')
                #continue
